...
NOTE: Il n'est pas nécessaire de redémarrer le serveur tomcat avec `gradle tomcatRun` pour que les modifications soient prise en compte dans les jsp.
...
Envoyer les données en JSON via une mécanique AJAX réglerait le problème.
