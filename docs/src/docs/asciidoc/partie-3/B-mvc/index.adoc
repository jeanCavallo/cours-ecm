...
C'est un pattern présent dans de nombreux languages et envirronements.

C'est une division du code en sous technologies adaptées à chaque aspect.
****
...
Spring MVC ne brille pas pour sa *convention over configuration* (en partie à cause de JEE et en parie à cause de Spring).

La configuration se divise en 3 parties.

=== web.xml

C'est le fichier de configuration standard JEE pour les applications web.
...
Un Controller est une classe java annoté avec `@Controller`
...
La liste les http://docs.spring.io/spring-framework/docs/current/spring-framework-reference/html/mvc.html#mvc-ann-arguments[paramètres supportés] par les handlers est disponible dans le doc de Spring MVC
...
NOTE: Spring converti "hello" en /WEB-INF/jsp/hello.jsp en se basant sur la configuration dans `spring-servlet.xml`

Retourner une chaîne de caractères est interprété comme un forward vers une vue.

La liste complète des http://docs.spring.io/spring/docs/current/spring-framework-reference/htmlsingle/#mvc-ann-return-types[retours possibles] est est disponible dans le doc de Spring MVC
...
Voici les annotations principales.
...
----
<1> `@RequestMapping` se cumule à tous les handlers du Controller
...
Afin de tester un Controller que celui ci soit en condition réelle, dans un conteneur Spring.
