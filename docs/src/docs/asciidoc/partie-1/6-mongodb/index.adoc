...
Lorsque l'on construit une application, on utilise un driver.
Selon le language et les frameworks, la façon d'utiliser mongo peut varier.
...
On pourrait utiliser le shell mais une interface graphique est plus simple pour débuter.

Il existe beaucoup de https://docs.mongodb.org/ecosystem/tools/administration-interfaces/[clients graphique]
