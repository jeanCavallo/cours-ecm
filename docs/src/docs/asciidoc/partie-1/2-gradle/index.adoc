...
La configuration de groovy est contenue dans un fichier *build.gradle* à la racine du projet.
...
Ou via la commande tree (`brew install tree`)
...
----
<1> on prend la version *4.12* de *junit* qui est dans le groupe *junit*.
...
Et bien sûr un https://plugins.gradle.org/[moteur de recherche] sur le site de gradle.
