...
En pratique, la commande de compilation devient rapidement très compexe. Fort heureusement, c'est l'IDE ou le système de build qui a la charge de générée et piloter celle ci.
...
Cela permet d'avoir plusieurs classes avec le même nom mais dans des packages différents.
...
Tous les autres types sont des objets héritant de la classe Object.
...
Une interface ne peut définir que des méthodes publiques et des constantes. Une classe peut alors 'implémenter' une interface. 
...
* on a le droit d'écrirer des méthodes statiques,
...
Comme son nom l'indique, elle sert à annoter (marquer) un autre élément, un peu comme un sticker.
...
Cela peut sembler un peu indigeste au départ mais c'est très utile et complet.

== gestion de la mémoire

La machine virtuelle java réserve et libère la mémoire : nous sommes dans un environnement managé.
