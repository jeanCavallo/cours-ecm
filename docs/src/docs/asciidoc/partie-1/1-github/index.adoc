...
.Précison
****
git est un *distributed VCS* (DVCS) et suit donc une philosophie particulière.

Il multiplie les capacités de travail au travers des branches (locales ou distantes) permettant de travailler simultanément sur plusieurs sujets en parallèle.
****
...
Certains fichiers ne doivent pas être envoyé vers le serveur git : 
...
<2> il y a souvent de l'aide en ligne de commande avec git
